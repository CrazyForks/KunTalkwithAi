...
import { Plus, Trash2, Edit2, Upload, Key, Server, ChevronDown, ChevronUp, Box, Check, RefreshCw } from 'lucide-react';
...
const INITIAL_CONFIGS: ApiConfig[] = [
  {
    ...
    provider: 'Gemini',
    name: '默认配置',
    baseUrl: import.meta.env.VITE_DEFAULT_TEXT_API_URL || import.meta.env.VITE_GOOGLE_API_BASE_URL || 'https://generativelanguage.googleapis.com',
    apiKey: import.meta.env.VITE_DEFAULT_TEXT_API_KEY || '',
    models: import.meta.env.VITE_DEFAULT_TEXT_MODELS ? (import.meta.env.VITE_DEFAULT_TEXT_MODELS as string).split(',') : ['gemini-3-flash-preview', 'gemini-2.5-flash', 'gemini-2.5-flash-lite'],
    channel: 'Gemini',
    isDefault: true,
    modality: 'TEXT',
  },
...
export const SettingsDialog: React.FC<SettingsDialogProps> = ({ isOpen, onClose }) => {
    ...
    (async () => {
      ...
      if (defaultConfig) {
          if ((import.meta as any).env?.DEV) {
              console.log('[SettingsDialog] Syncing default config. Env models:', envModelsStr);
          }
          ...
          const targetModels = envModelsStr
              ? (envModelsStr as string).split(',').map(m => m.trim()).filter(Boolean)
              : ['gemini-3-flash-preview', 'gemini-2.5-flash', 'gemini-2.5-flash-lite'];

          if (JSON.stringify(defaultConfig.models) !== JSON.stringify(targetModels)) {
  ...
  const handleAddConfig = async (newConfig: { provider: string; address: string; key: string; channel: string; toolsJson?: string; models?: string[] }) => {
  ...
  const handleEditConfirm = async (newConfig: { provider: string; address: string; key: string; channel: string; toolsJson?: string; models?: string[] }) => {
...
const ConfigCard = ({ config, isSelected, onSelect, onEdit, onDelete, onAddModel, onRemoveModel, onFetchModels }: ConfigCardProps) => {
  ...
  return (
    <div
      onClick={onSelect}
      className={`relative overflow-hidden transition-all duration-200 group cursor-pointer border rounded-2xl ${isSelected ? 'bg-emerald-950/20 border-emerald-500/50 ring-1 ring-emerald-500/30' : 'bg-zinc-900 border-zinc-800 hover:border-zinc-700'}`}
    >
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl transition-colors ${isSelected ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-400'}`}>
              {config.isDefault ? <Box size={20} /> : <Server size={20} />}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className={`text-base font-bold transition-colors ${isSelected ? 'text-emerald-100' : 'text-zinc-100'}`}>
                  {config.name}
                </h3>
              ...
              <div className="flex items-center gap-2 mt-1">
                {config.id !== 'text_default_gemini' ? (
                  <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded border transition-colors ${isSelected ? 'bg-emerald-950/30 border-emerald-500/20' : 'bg-zinc-950/50 border-zinc-800/50'}`}>
                    <Key size={10} className={`${isSelected ? 'text-emerald-400/70' : 'text-zinc-500'}`} />
                    <span className={`text-xs font-mono ${isSelected ? 'text-emerald-200/70' : 'text-zinc-400'}`}>{config.apiKey ? '已设置' : '未设置'}</span>
                  </div>
                ) : (
                  <div className={`flex items-center gap-1.5 text-xs ${isSelected ? 'text-emerald-300/80' : 'text-zinc-500'}`}>
                    <Box size={10} />
                    <span>内置模型</span>
                  </div>
                )}
        ...
        <div
          className={`
            ...
            ${isSelected ? 'bg-emerald-950/30 border-emerald-500/20' : 'bg-zinc-950/50 border-zinc-800/50'}
          `}
        ...
        >
          ...
          >
            <div className={`flex items-center gap-2 text-xs font-medium uppercase tracking-wider ${isSelected ? 'text-emerald-400/60' : 'text-zinc-500'}`}>
              <Box size={12} />
            ...
            <div className="flex items-center gap-2">
              {config.id !== 'text_default_gemini' && (
                <>
                  <button
                    onClick={handleFetch}
                    className={`p-1 transition-colors ${isSelected ? 'text-emerald-400/60 hover:text-emerald-300' : 'text-zinc-500 hover:text-zinc-300'}`} title="刷新模型列表"
                  >
                    <RefreshCw size={14} className={isFetching ? 'animate-spin' : ''} />
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); onAddModel(); }}
                    className={`p-1 transition-colors ${isSelected ? 'text-emerald-400/60 hover:text-emerald-300' : 'text-zinc-500 hover:text-zinc-300'}`} title="添加模型"
                  >
                    <Plus size={14} />
                  </button>
                </>
              )}
              <button className={`transition-colors ${isSelected ? 'text-emerald-400/60 hover:text-emerald-300' : 'text-zinc-600 hover:text-zinc-400'}`}>
              ...
              >
                <div className="flex flex-wrap gap-2 pt-1">
                    ...
                    >
                      ...
                      {config.id !== 'text_default_gemini' && (
                        <button
                          ...
                          className={`opacity-0 group-hover:opacity-100 p-0.5 rounded-full transition-all ${isSelected ? 'hover:bg-emerald-800 text-emerald-400' : 'hover:bg-zinc-700 text-zinc-500 hover:text-red-400'}`}
                        >
                          <XIcon size={10} />
                        </button>
                      )}
                  ...
                  {config.models.length === 0 && (
                    <div className={`w-full text-center py-2 text-xs italic ${isSelected ? 'text-emerald-500/50' : 'text-zinc-600'}`}>No models configured</div>
                  )}
                </div>
              </motion.div>
            )}
